...
class CrsTemplateController extends AmsPaPa 
{
	...
	{
		$data = CrsRole::where('type',2)->with('UsersData')->get()->toArray();
		return View::make('Fantasy.ams.crs_template.index',
		[
			'data' => $data
		]);
	}
	...
	{
		$data = CrsRole::where('type',2)->with('UsersData')->get()->toArray();
		return View::make('Fantasy.ams.crs_template.ajax.table',
		[
			'data' => $data
		]);
	}
